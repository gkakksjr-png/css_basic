...
* 자식이 2개 이상일 때 부모에 속성을 적용하는 것으로 자식에게 공통 값을 적용할 수 있다.
    * `ul-li*5` 관계 시 `ul {color:rad}` 부모 ul의 color를 li 자식에게 상속시킨다.
    * 꾸미려는 css 속성이 글자에 관련된 속성(글꼴, 글자크기, 글자색상) 본인에게 직접 적용하는 것을 권장함.(우선순위가 꼬여서)
## 글꼴 사용 시 주의사항
...
* `margin` : 바깥쪽(형제 사이) 여백(피그마 기준 **간격**)
    ...
    * **위 4개 속성 방향은 padding과 동일**
    * `margin:0 auto` : 레이아웃 가운데배치 (너비가 화면보다 작아야함)
# table HTML+CSS
* `table > tr > th or td`
* `table {width:300}` 열이 3개일 경우 100씩 자동 분배
...
* `content:''` -> 내용인식속성, 글자 필요한 디자인이 아닐 경우 '' 빈 따옴표
